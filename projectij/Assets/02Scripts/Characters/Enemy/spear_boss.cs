        ...
        if (player != null)
        {
            ...
            if (!beingDied)//죽는 애니메이션 실행중 아닐때
            {
                ...
                if (playerPos.x > transform.position.x)
                {
                    ...
                    transform.GetChild(1).GetComponent<BoxCollider2D>().offset = new Vector2(1.29f, 0.344149f);
                }
                else
                {
                    spriteRenderer.flipX = false;
                    transform.GetChild(1).GetComponent<BoxCollider2D>().offset = new Vector2(-2.455937f, 0.344149f);
                }
